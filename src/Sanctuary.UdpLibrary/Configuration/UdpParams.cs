...
public class UdpParams
{
    ...
    // the main loop and call DumpPacketHistory when it is triggered.  This will dump a history of the last
    ...
    // this setting can be overridden on a per-connection basis by changing NoDataTimeout
    // default = 0
    ...
    // for other reasons.  When disconnected due to this, the disconnect reason is set to UnacknowledgedTimeout
    // default = 90000
    ...
    // before it will terminate the connection with a ReliableOverflow.  If this value is set to
    ...
    // itself using TotalPendingBytes and act as appropriate.
    // default = 0
    ...
    // GiveTime is called, every UdpConnection object gets processing time as well.
    ...
    // you need to specify the size of the packets that are in the pool.  Then, when somebody calls CreatePacket,
    ...
    // default = 1000, (0 = don't use memory pooling at all)
    ...
    // If you call ThreadStart to start a background thread going calling GiveTime, it will sleep for
    ...
    // by the application through the EventQueuing .  When in event queuing mode, all callbacks
