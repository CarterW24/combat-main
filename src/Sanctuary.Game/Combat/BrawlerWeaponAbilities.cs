...
public static class BrawlerWeaponAbilities
{
    ...
    public static AbilityPacketSetDefinition BuildToolbar(Player player, IResourceManager resources)
    {
        ...
        if (weapon is null)
            return new AbilityPacketSetDefinition { ProfileId = BrawlerProfileId };
        ...
        var def = new AbilityPacketSetDefinition { ProfileId = BrawlerProfileId };

        def.AbilitySet.Abilities[0] = MakeSlot(MeleeSlotDefId, weapon.Melee.IconImageId, nameId, manaCost: 0);
        def.AbilitySet.Abilities[1] = MakeSlot(SpecialSlotDefId, weapon.Special.IconImageId, nameId, manaCost: SpecialEnergyCost);
