...
public static class HeldPowerupProbe
{
    ...
    private const uint ShieldBuffNameId = 1352849523; // locale "Shield" — buff-bar label
    private const int ShieldBuffMs = 8000;      // mirror the arena's invuln window
    ...
    public static bool TryUse(Player player, IResourceManager resources)
    {
        ...
        // Shield gets the buff-bar tag here too (icon + 8s duration pie) so the effect-tag flow is
        // testable overworld; the arena's version adds the real invuln + speed on top.
        if (kind == "shield")
