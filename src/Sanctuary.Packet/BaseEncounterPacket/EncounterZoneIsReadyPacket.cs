...
// "MiniGame:SetMiniGameReady" event — i.e. it flips the spinner into the green GO! button and renders the detail.
// sub_9B0CC0 takes NO packet fields (it ignores the contents) — so to trigger it we just need to send a sub107
...
// BaseEncounter header — NOTHING ELSE. sub_A9F990 reads only the header (sub_8D6690 = [short op][short sub]
// [int Unknown][int Unknown2]) then returns `a4 || byEnd-byData <= 0`; the dispatch passes a4=0, so it requires
// ZERO leftover bytes. (Our earlier defensive +8 bytes left 8 unconsumed → returned false → sub_9B0CC0 never
// ran → spinner never cleared. Lesson: this validator fails on UNDER-read AND on any leftover.)
...
public class EncounterZoneIsReadyPacket : BaseEncounterPacket, ISerializablePacket
{
    ...
    public byte[] Serialize()
    {
        ...
        Write(writer);       // [op41][sub107][int Unknown][int Unknown2] — the ENTIRE packet (12 bytes)
